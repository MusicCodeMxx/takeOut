...
import top.starshine.commons.model.redis.RedisCommonKey;
...
public class TokenInterceptor implements HandlerInterceptor {
    ...
    public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object handler) {
        ...
        /* 检查是否黑名单,是黑名单就直接抛异常 */
        Object obj = redisTemplate.opsForHash().get(RedisCommonKey.LOGIN_BLACK_LIST, id);
        if (null != obj) throw new UnauthorizedException(TokenStatus.YOU_ARE_LOGGED_OUT);

        User user = null;
        try {
            /* 获取缓存中的用户信息 */
            user = (User) redisTemplate.opsForValue().get(RedisCommonKey.LOGIN_USER_DETAILS + id);
            if (null == user) {
                // 缓存中的用户信息过期
                user = loadUserDetailService.getById(id);
                // 获取为空, 抛空用户异常
                if (null == user) throw new UnauthorizedException(BasisStatus.UNAUTHORIZED);
                // 重新存入缓存
                redisTemplate.opsForValue().set(
                        RedisCommonKey.LOGIN_USER_DETAILS + user.getId(),
                        user,
                        RedisCommonKey.CACHE_MINUTES_30, TimeUnit.MINUTES);
            }
        } catch (Exception e) {
            e.printStackTrace();
