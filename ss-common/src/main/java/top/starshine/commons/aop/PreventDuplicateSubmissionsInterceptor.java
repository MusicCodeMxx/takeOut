...
public class PreventDuplicateSubmissionsInterceptor {
    ...
    public void interceptor(PreventDuplicateSubmissions preventDuplicateSubmissions) {
        ...
        try {
            ...
            stringBuilder.append(request.getRequestURI()).append(":").append(request.getRemoteHost()); // Limiting:url:主机地址
            User user = ThreadLocalCache.get();
            // 检查是否有登录用户信息
            if (null != user) stringBuilder.append(":").append(user.getId()).append(":").append(user.getPhoneNumber());// Limiting:url:主机地址：userid:手机号
            ...
            //初始设置过期时间
            if (increment == 0 && expire > 0) entityIdCounter.expire(expire, TimeUnit.SECONDS);         // 设置过期时间 3 秒
        } catch (Exception e) {
            e.printStackTrace();
            return ;
        }
